...
<form action="" method="post" align="center">
    ...
    <!--5/25削除-->
<div align="center">
    <fieldset><legend>会員情報（変更不可）</legend>
    <label>
    ...
    <strong>ERROR {{$errors->first('num_people')}}</strong>
    @endif
    ...
    <strong>ERROR {{$errors->first('checkin_time')}}</strong>
    @endif
    @if($errors->has('checkout_time'))
    <strong>ERROR {{$errors->first('checkout_time')}}</strong>
    @endif
    <label>
        <p>チェックイン予定時間:<input type="datetime-local" name="checkin_time" value="{{$tomorrow}}" required>
            <br><!--min="06:00" max="18:00"-->
